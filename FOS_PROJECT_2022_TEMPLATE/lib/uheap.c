...
void initialize_dyn_block_system()
{
	...
	sys_allocate_chunk(startAdd, memblock_size,PERM_WRITEABLE | PERM_PRESENT |PERM_USED | PERM_USER); // allocate heap
...
//		memset(MemBlockNodes[i], 0 , sizeof(struct MemBlock)); // may error
	...
	initialize_MemBlocksList(MAX_MEM_BLOCK_CNT); // ms1
	//[4] Insert a new MemBlock with the heap size into the FreeMemBlocksList
	blockToInsert = LIST_FIRST(&(AvailableMemBlocksList));
	LIST_REMOVE(&(AvailableMemBlocksList), blockToInsert);

	blockToInsert->sva = USER_HEAP_START;
	blockToInsert->size = NUM_OF_UHEAP_PAGES * PAGE_SIZE;
...
//		cprintf("1 - va = %x \t size = %d\n",allocated_block_FF->sva, allocated_block_FF->size);
		if(allocated_block_FF != NULL)
		{
			insert_sorted_allocList(allocated_block_FF); // updated
//			cprintf("PASSED\n");
...
//	cprintf("1 - va = %x \t size = %d\n",BLOCK->sva, BLOCK->size);
	if( BLOCK != NULL )
	{
//		cprintf("USER\n");
//		cprintf("2 - va = %x \t size = %d\n",BLOCK->sva, BLOCK->size); // correct sva & size
		sys_free_user_mem(BLOCK->sva, BLOCK->size);
		LIST_REMOVE(&(AllocMemBlocksList),BLOCK);
		insert_sorted_with_merge_freeList(BLOCK);
//		cprintf("3 - va = %x \t size = %d\n",BLOCK->sva, BLOCK->size); // zero sva & size (after REMOVE)
...
//			cprintf("PASSED\n");
			insert_sorted_allocList(allocated_block_FF); // updated
			int index =sys_createSharedObject(sharedVarName,size,isWritable,(void*)allocated_block_FF->sva);
			if(index>=0)
			{
				sharedarr[index]=allocated_block_FF->sva;
//				cprintf("size : %d \n",allocated_block_FF->size);
				return (void*)allocated_block_FF->sva;
			}
		}
	 }
...
void* sget(int32 ownerEnvID, char *sharedVarName)
{
	...
	int shr_objSize =sys_getSizeOfSharedObject(ownerEnvID,sharedVarName);

	if(shr_objSize ==(int)NULL)
	{
		return (void*)NULL;
	}
	else
	{
		if(sys_isUHeapPlacementStrategyFIRSTFIT())
		{
			shr_objSize= ROUNDUP((int)shr_objSize,PAGE_SIZE);
			struct MemBlock *allocated_block_FF = alloc_block_FF(shr_objSize);
			insert_sorted_allocList(allocated_block_FF); // updated
			if(allocated_block_FF !=NULL)
			{
//							cprintf("PASSED\n");
				int add = sys_getSharedObject(ownerEnvID,sharedVarName,(void*)allocated_block_FF->sva);
				if(add >= 0)
				{
					sharedarr[add]=allocated_block_FF->sva;
//					cprintf("size : %d \n",allocated_block_FF->size);
					return (void*)allocated_block_FF->sva;
				}
			}
...
void sfree(void* virtual_address)
{
	...
	uint32 id=(uint32) NULL;
	for(int i =0;i<100;i++)
	{
//		cprintf("arr :%d \n",sharedarr[i]);
		if(sharedarr[i]== (uint32)virtual_address)
		{
			id=i;
		//	cprintf("iiiiiiiiiiiiiiii : %d \n",i);
			break;
		}
	}
