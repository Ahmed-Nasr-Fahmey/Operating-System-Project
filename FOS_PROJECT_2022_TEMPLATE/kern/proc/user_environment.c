...
//	panic("env_free() is not implemented yet...!!");
	...
	{
		if(!env_page_ws_is_entry_empty(e, i)) // return 0 if not empty
		{
			uint32 va = env_page_ws_get_virtual_address(e, i);
			unmap_frame(e->env_page_directory, va); // mem
			env_page_ws_clear_entry(e, i); // ws
			kfree((void * ) va);
...
//			}
	...
	// [4] free the MemBlockNodes array of the USER HEAP dynamic allocator [if exists] // allocatedList
//	kfree((void * ) MemBlockNodes->sva);
//	free((void * ) USER_DYN_BLKS_ARRAY);

	struct FrameInfo* DFI;
	uint32* DPT = NULL;
	DFI = get_frame_info(e->env_page_directory, USER_DYN_BLKS_ARRAY, &DPT);
	if (DFI)
	{
		uint32 memblock_size = ROUNDUP(NUM_OF_UHEAP_PAGES * sizeof(struct MemBlock), PAGE_SIZE);
		uint32 startAdd = USER_DYN_BLKS_ARRAY;
		uint32 endAdd = startAdd + memblock_size;
...
//	for (uint32 va = 0x7fdff000; va < USER_HEAP_START; va += PAGE_SIZE)
//	{
//		struct MemBlock * block = NULL;
//		block = find_block(&AvailableMemBlocksList,va);
//		if(block != NULL)
//		{
//			LIST_REMOVE(&AvailableMemBlocksList, block);
//		}
//		block = find_block(&FreeMemBlocksList,va);
//		if(block != NULL)
//		{
//			LIST_REMOVE(&FreeMemBlocksList, block);
//		}
//		block = find_block(&AllocMemBlocksList,va);
//		if(block != NULL)
//		{
...
//	free_user_mem(e, MemBlockNodes->sva, MemBlockNodes->size);
//	free_user_mem(e, 0x7fdff000, USER_HEAP_START - 0x7fdff000);

	// [5] Free Shared variables [if any]
//	kfree((void * ) e->ptr_sharing_variables);
	for (int i = 0; i < MAX_SHARES; i++) // if semaphore exist with the same ownerID & name
	{
		if (shares[i].ownerID == e->env_id)
		{
			free_share_object(i);
		}
	}
	...
	for (int i = 0; i < MAX_SEMAPHORES; i++) // if semaphore exist with the same ownerID & name
	{
		if (semaphores[i].ownerID == e->env_id)
		{
	...
	// [7] Free all TABLES from the main memory
		...
		for (uint32 i = 0; i<USER_TOP; i+=PTSIZE)
		{
			kfree((void*)kheap_virtual_address(e->env_page_directory[PDX(i)]));
		}


//	for (uint32 va = 0; va < USER_TOP; va += 1024 * PAGE_SIZE)
//	    {
...
//	#else
//	            {
//	                struct FrameInfo* ptr_table_frame = to_frame_info(e->env_page_directory[PDX(va)] & 0xFFFFF000);
//	                free_frame(ptr_table_frame);
//	                ptr_table_frame->references=0;
//	            }
//	#endif
...
//	for(int i = 0; i < 1024; i++) // num of entries
//	{
//		if(e->env_page_directory[i] != 0) // check page table entries
//		{
////			cprintf("j %d\n", j);
//			kfree((void*)e->env_page_directory[i]);  // free page table
//		}
//	}
...
//
//	// [2] Free the pages in the PAGE working set from the main memory
//	uint32 WsMaxSize = e->page_WS_max_size;
//	for (uint32 i=0; i<WsMaxSize; i++)
//	{
//		if (env_page_ws_is_entry_empty(e, i)) continue;
//		unmap_frame(e->env_page_directory, env_page_ws_get_virtual_address(e, i));
//	}
//
//	// [3] free the PAGE working set itself from the main memory
//	if (USE_KHEAP)
...
//	if (DFI)
//	{
//		uint32 MemBlockNodes_size = ROUNDUP(NUM_OF_UHEAP_PAGES * sizeof(struct MemBlock), PAGE_SIZE);
//		uint32 cva = USER_DYN_BLKS_ARRAY;
//		uint32 eva = cva + MemBlockNodes_size;
//		for (; cva<eva; cva+=PAGE_SIZE)
...
//	curenv = e;
//	for (uint32 i=0; i<e->ENV_MAX_SHARES; i++)
//	{
//		if (e->ptr_sharing_variables[i].size)
//		{
//			uint32 ShareObjectID = e->ptr_sharing_variables[i].id_in_shares_array;
//			uint32 sva = e->ptr_sharing_variables[i].start_va;
//			freeSharedObject(ShareObjectID, (void*)sva);
//		}
//	}
//	// Switch back to the current environment
//	curenv = current_env;
//
//	if (USE_KHEAP)
//		kfree(e->ptr_sharing_variables);
...
//			free_semaphore_object(get_semaphore_object_ID(e->env_id, semaphores[i].name));
//
//	// [7] Free all TABLES from the main memory
//	uint32 current_table = 0;
//	uint32 last_table = USER_TOP;
//	for (; current_table<last_table; current_table+=PTSIZE)
//		if (USE_KHEAP && (e->env_page_directory[PDX(current_table)]&PERM_PRESENT))
//			kfree((void*)kheap_virtual_address(EXTRACT_ADDRESS(e->env_page_directory[PDX(current_table)])));
//	tlbflush();
//
//	// [8] free the page DIRECTORY from the main memory
//	if (USE_KHEAP)
