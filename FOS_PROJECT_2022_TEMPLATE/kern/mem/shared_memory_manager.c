...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	if(exist>=0)
	{
	 ...
	 {
		 struct Share* object;
		 int sharedObjectID =allocate_share_object(&object);
...
//			cprintf("ELSE\n");
			...
			strcpy(shares[sharedObjectID].name,shareName); // char* strcpy(char* destination, const char* source);
			...
			uint32 x =0;
			for(uint32 i = (uint32)virtual_address; i < ROUNDUP((uint32)virtual_address+size,PAGE_SIZE); i+= PAGE_SIZE)
			{
...
//				 cprintf("va = %x\n",i);
				 ...
				 {
					 add_frame_to_storage(shares[sharedObjectID].framesStorage,ptr_frame_info,x);
					 x +=1;
				 }
			}
			return sharedObjectID;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	if(index == E_SHARED_MEM_NOT_EXISTS){
		...
		uint32 x =0;
		for(uint32 i = (uint32)virtual_address; i <ROUNDUP((uint32)virtual_address+shares[index].size,PAGE_SIZE);i+= PAGE_SIZE)
		{
//			cprintf("va = %x\n",i);
			struct FrameInfo* frame= get_frame_from_storage(shares[index].framesStorage,x);
			if(shares[index].isWritable == 1)
				map_frame(myenv->env_page_directory,frame,i,PERM_WRITEABLE | PERM_USER);
			else
				map_frame(myenv->env_page_directory,frame,i,PERM_USER);
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	...
	int f = get_share_object_ID(shares[sharedObjectID].ownerID,shares[sharedObjectID].name);

	if(f == E_SHARED_MEM_NOT_EXISTS)
	{
//		cprintf("not exist");
		return E_SHARED_MEM_NOT_EXISTS;
	}
		...
		int x=0;
		for(uint32 i = (uint32)startVA; i < ROUNDUP(((uint32)startVA + shares[sharedObjectID].size), PAGE_SIZE); i +=  PAGE_SIZE)
		{
			uint32 *ptr_page_temp = NULL;
			struct FrameInfo *ptr_frame_info = NULL;
			ptr_frame_info = get_frame_from_storage(shares[sharedObjectID].framesStorage,x);
			x++;
		...
		for(uint32 i = (uint32)startVA; i < ROUNDUP(((uint32)startVA + shares[sharedObjectID].size), PAGE_SIZE); i +=  PAGE_SIZE)
		{
			...
			if(!flag) // if all entries in page table = zero
			{
				...
				myenv->env_page_directory[PDX(i)] = 0; // set directory entry to zero
			}
		}
