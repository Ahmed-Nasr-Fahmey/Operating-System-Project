...
//		{
//			if(semaphores[i].name == semaphoreName)
//			{
//				return E_SEMAPHORE_EXISTS;
//			}
//		}
//	}
	int index = get_semaphore_object_ID(ownerEnvID,semaphoreName); // if semaphore exist with the same ownerID & name
	...
	else
	{
		...
		int ID = allocate_semaphore_object(&allocatedObject);
		if(ID != E_NO_SEMAPHORE) // if allocation succeed
		{
			allocatedObject->ownerID = ownerEnvID;
//			allocatedObject->name = semaphoreName;
			strcpy(allocatedObject->name, semaphoreName); // char* strcpy(char* destination, const char* source);
...
void waitSemaphore(int32 ownerEnvID, char* semaphoreName)
{
	...
	if(x >= 0)
	{
		...
		{
			enqueue(&semaphores[x].env_queue,myenv);
			myenv->env_status=ENV_BLOCKED; // set status
			curenv=NULL;
			fos_scheduler();
		}
	}
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if(x >= 0)
	{
		...
		if(semaphores[x].value<=0)
		{
			struct Env* myenv = dequeue(&semaphores[x].env_queue);
			enqueue(env_ready_queues,myenv);
			myenv->env_status= ENV_READY; // set status
		}
	}
